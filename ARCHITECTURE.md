...
│  │  ├─ /trip/:tripId               → TripPage（行程總覽）   │  │
│  │  │  ├─ /stop/:entryId           → StopDetailPage         │  │
│  │  │  ├─ /stop/:entryId/map       → MapPage（單點全圖）    │  │
│  │  │  └─ /map[?day=N]             → MapPage（日／全行程）  │  │
...
| Path | Component | 用途 |
|------|-----------|------|
| `/trip/:tripId` 或 `/` | `TripLayout` + `TripPage` | 旅伴瀏覽行程（唯讀），Layout 提供 `TripContext` 共用 trip+days fetch |
| `/trip/:tripId/stop/:entryId` | `StopDetailPage` | 景點詳情頁（editorial hero + 導航 + 餐廳正備選 + 必買購物）|
| `/trip/:tripId/map[?day=N]` | `MapPage` | 全圖地圖頁（overview mode + Funliday 風 day tabs + swipe cards）|
| `/trip/:tripId/stop/:entryId/map` | `MapPage` | 景點專屬全圖頁（detail mode 聚焦單 pin）|
...
`TripLayout` 是 React Router 嵌套 layout：`/trip/:tripId/*` 所有子路由共用同一份 trip+days fetch（透過 `TripContext`），避免 StopDetailPage / MapPage 重複抓。
