---
name: tp-patch
description: "Use when the user wants to bulk-fill missing POI data fields (google_rating, maps, address, phone, hours) across one or more trips via web search — e.g. '批次補欄位', '所有行程補 rating', '跨行程補 POI 資料', '/tp-patch --target hotel --field all'. This is for systematic field-level gaps, not content changes. For changing a specific restaurant or adding a stop, use /tp-edit. For fixing R0-R18 violations holistically, use /tp-rebuild."
user-invocable: true
---
...
1. 取得目標行程清單（Windows 環境用 node，不用 curl — 見 tp-shared/references.md）：
   ```bash
   # 讀取操作可用 curl（GET 無中文 body 問題）
   curl -s "https://trip-planner-dby.pages.dev/api/trips?all=1" \
     -H "CF-Access-Client-Id: $CF_ACCESS_CLIENT_ID" \
     -H "CF-Access-Client-Secret: $CF_ACCESS_CLIENT_SECRET"
   ```
   > ⚠️ 寫入操作（Phase 3 PATCH）必須用 `node writeFileSync` + `--data @file` 避免中文亂碼。詳見 tp-shared/references.md curl 模板。
