...
5b. **R19 每日首 entry 檢查（鐵律，見 tp-quality-rules）**：驗證每天 `timeline[0]` 是否合 R19。
    ...
    - Day N（N ≥ 2）首 entry 應指向 Day N-1 `day.hotel` 同 POI，`title` 含 check-out 語意；不合則**在 timeline 頂端插入** leading entry（time 優先用 Day N-1 `hotel.checkout`，無則 `"07:00"`），不複製 `hotel.infoBoxes`
