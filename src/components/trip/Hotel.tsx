...
export const Hotel = memo(function Hotel({ hotel }: HotelProps) {
  ...
  return (
    <>
      <div className="col-row" onClick={toggle} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(); } }} style={{ cursor: 'pointer' }} aria-expanded={open} role="button" tabIndex={0}>
