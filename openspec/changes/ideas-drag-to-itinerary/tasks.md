...
- [ ] 8.4 `/design-review` drag hover state 視覺對齊 DESIGN.md — **manual ops gate**：需人工跑 `/design-review` 對 drag-overlay shadow / day badge is-over / slot chip is-over hover 視覺校對 Ocean palette
- [ ] 8.5 `/cso --diff` 驗 batch API 無 SQL injection — **manual ops gate**：需跑 `/cso --diff master..HEAD` 對 `functions/api/trips/[id]/entries/batch.ts` audit。code 內部已用 prepared statement + bind 參數化 + ownership 預檢，無動態 SQL 字串拼接
- [ ] 8.6 `/tp-team` pipeline — **manual ops gate**：tp-code-verify → review → cso → qa → ship pipeline 由 user 觸發
- [ ] 8.7 staging → prod ship — **manual ops gate**：proposal status「Deferred to V2」，ship 由 V2 排程進來時觸發；當前 change 留 active
